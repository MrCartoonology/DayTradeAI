...
def correlate_yesterday_pdiff_with_today(df):
    ...
    for x, y in pdiffs.items():
        print(x, len(y))
    quantiles = dict(
        lower=np.percentile(pdiffs["_today_pdiff_1d"], 10),
        upper=np.percentile(pdiffs["_today_pdiff_1d"], 90),
    )
    print(quantiles)
    idxes = dict(
        low=pdiffs["_today_pdiff_1d"] < quantiles["lower"],
        high=pdiffs["_today_pdiff_1d"] > quantiles["upper"],
    )
