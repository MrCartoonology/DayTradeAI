...
def preprocess_data(df: pd.DataFrame, data_cfg: Dict[str, Any], preprocess_cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    for lag_feat in preprocess_cfg['lag_feats']:
        df = add_lag_feat(df, tickers_plus_cash, lag_feat, preprocess_cfg['trading_day_lags'])
...
def label_beat_index_1d(df: pd.DataFrame, stocks: List[str], preprocess_cfg: Dict[str, Any]) -> pd.DataFrame:
    """
    ...
    for stock in stocks:
        df[f'label_{stock}'] = (df[f'label_{stock}_pdiff_1f'] > index_performance).astype(int)
